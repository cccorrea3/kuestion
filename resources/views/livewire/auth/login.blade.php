<div class="min-h-[80vh] flex items-center justify-center py-12 px-4">
    <div class="w-full max-w-sm">
        ...
        <div class="text-center mb-8">
            ...
            <h1 class="text-2xl font-bold text-text tracking-tight">Inicia sesión</h1>
            <p class="text-sm text-text-muted mt-1.5">Accede a tu panel de vigilancia</p>
        </div>

        <form wire:submit="authenticate" class="bg-surface rounded-2xl shadow-sm border border-border p-8 space-y-5">
            {{-- Decorative top bar --}}
            <div class="absolute top-0 left-8 right-8 h-0.5 bg-gradient-to-r from-primary/40 via-accent/40 to-primary/40 rounded-full -translate-y-0.5 hidden sm:block"></div>
            ...
            <p class="text-center text-sm text-text-muted pt-1">
                ¿No tienes cuenta?
                <a href="{{ route('register') }}" wire:navigate class="text-accent hover:text-orange-600 font-semibold transition-colors duration-150">Regístrate</a>
            </p>
        </form>
    </div>
</div>
