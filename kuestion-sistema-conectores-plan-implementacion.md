...
- **Contrato confirmado (P3, Ingeniería de Kuaforia):**
  ...
  - **Respuesta exitosa:** `result.content[0].text` es un **STRING JSON** (no objeto anidado directo). **Hallazgo de pruebas E2E (agosto 2026):** la implementación REAL de Kuaforia devuelve `tenant`, `default_workspace`, etc. al **nivel raíz** del string JSON, **sin wrapper `data`** (forma documentada originalmente como `{"success": true, "data": {"tenant": ...}}`). El `IdentityResolver` normaliza y acepta **ambas formas** (root y `data.*`): `$data = $decoded['data'] ?? $decoded;`. Usar `tenant.slug` y `tenant.name` desde la forma normalizada.
