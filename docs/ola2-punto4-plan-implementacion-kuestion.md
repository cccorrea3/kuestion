...
|---|---|---|---|
| A.1 | Contrato mínimo de metadatos **confirmado (RESPUESTAS_OLA2_KUESTION.md Q4.1–Q4.5)**: viajan **por nodo**, en detalle + listado + `POST /contribute` (análisis síncrono), snake_case. | Coordinación con QuBeKa (no bloquea el desarrollo: se trabaja con mock del contrato §2.3). | Contrato escrito y acordado (sección 4, B1/B2). |
...
| A.4 | Manejo del caso **sin metadata** (sesiones previas al despliegue o campos ausentes): flag explícito `sin_detalle` en la normalización para que la UI muestre el estado de degradación sin romper. **Regla de precedencia confirmada (Q4.5):** si `explicacion` es `null`, nunca caer a `confianza`/`justificacion_ia` como fuente de display. | A.2 | Comportamiento definido y testeado para datos viejos. |

**Entregable verificable:** con el contrato simulado (Http fake), `getSession()` devuelve los nodos con su explicación normalizada (o el flag `sin_detalle`), y la suite de `QbkContributionService` queda en verde. **La confirmación de QuBeKa ya llegó (Q4.4/Q4.5, 2026-09-09; viabilidad confirmada en Q4.1) — Fase A desbloqueada.**
...
| B2 | **RESUELTA (2026-09-09, Q4.5 de QuBeKa — `RESPUESTAS_OLA2_KUESTION.md`):** los metadatos viajan **por nodo**, en los 3 endpoints (§7.2 del contrato: detalle, listado, `POST /contribute`), en snake_case. Para display usar **exclusivamente `explicacion.*`**: `explicacion.reasons[]` + `explicacion.confidence` reemplazan en la práctica a `justificacion_ia`/`confianza` para UI. Si `explicacion` es `null`, mostrar el fallback — **nunca caer a `confianza`/`justificacion_ia` como fuente alternativa** (mezclaría semánticas entre sesiones nuevas y viejas). Garantía de consistencia: ambos se persisten desde la misma respuesta del LLM (no pueden divergir por construcción); si por forma difieren, gana `explicacion.confidence` para display. | Resuelta |
| B3 | **RESUELTA (2026-09-09, Q4.4 de QuBeKa — `RESPUESTAS_OLA2_KUESTION.md`):** **fuera de alcance de v1, como decisión declarada (no vacío).** Evidencia verificada en el código de QuBeKa: `id_nodo_real_match` se persiste pero ningún consumidor lo usa, y la promoción crea un nodo nuevo por cada nodo propuesto (sin dedup contra el grafo real) — la decisión "vincular vs. crear nueva" no existe hoy, no hay nada que explicar. El "¿Por qué?" de v1 explica la **clasificación del nodo** (tipo, confianza, alternativas de tipo), no su relación con el grafo. Si en una versión futura la promoción consume `id_nodo_real_match`, `vinculacion_reason` se incorpora en ese mismo cambio. | Resuelta |
...
| Explicación de la vinculación a una Q existente (`vinculacion_reason`) | Decisión Q4.4: la decisión de vinculación no existe hoy en el pipeline de QuBeKa (`id_nodo_real_match` se persiste sin consumidor; la promoción no hace dedup contra el grafo real). El "¿Por qué?" de v1 explica la clasificación del nodo, no la relación con el grafo. Se incorporará junto a la promoción con vínculo real, si llega. |
